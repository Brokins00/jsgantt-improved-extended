...
const workingDays = date_utils_1.DEFAULT_WORKING_DAYS;
// Festività di test
const holidays = [
    {
        id: 'test-holiday',
...
const workingDaysOriginal = date_utils_1.countWorkingDaysInPeriod(taskStart, taskEnd, workingDays, holidays);
console.log(`Giorni lavorativi nel periodo originale: ${workingDaysOriginal}`);
// Calcola la nuova data di fine
const adjustedEndDate = date_utils_1.calculateAdjustedEndDate(taskStart, taskEnd, workingDays, holidays);
...
while (currentDate <= adjustedEndDate) {
    ...
    const isHoliday = holidays.some(h => currentDate.getTime() >= h.startDate.getTime() &&
...
const adjusted2 = date_utils_1.calculateAdjustedEndDate(task2Start, task2End, workingDays, holidays);
...
const date_utils_2 = require("../src/utils/date_utils");
const task3End = date_utils_2.addWorkingDays(task3Start, 4, workingDays, holidays); // 4 perché includiamo il giorno di inizio
